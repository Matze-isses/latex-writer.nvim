...
local function search_inline(inputString)
    ...
    for _, pattern in ipairs(tex_regex) do
        ...
        for _ in string.gmatch(inputString, pattern.regex) do
            start_index, end_index = string.find(inputString, pattern.regex, 1)
            ...
            inputString = string.sub(inputString, end_index, #inputString)
        end
    end
    return matches
end
...
print(search_inline("test \\( \\sum_{k=1}^n X(\\omega_k) \\) Would be the probability measure of a discrete random variable \\(X: \\Omega \\to \\mathbb R\\)")[1].tex)
